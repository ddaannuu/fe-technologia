...
export default {
  ...
  methods: {
    async fetchProduct() {
      try {
        const res = await fetch(`${API}/products/get_on_sale_by_id_api/${this.$route.params.id}`, {
          credentials: 'include'
        })
    ...
    async handleSubmit() {
      ...
      try {
        const res = await fetch(`${API}/products/update_on_sale_api/${this.$route.params.id}`, {
          method: 'POST',
